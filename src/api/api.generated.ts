// AUTO-GENERATED FROM /Users/skylerberg/Code/critical-path-api/.claude/worktrees/pending-invitations/openapi.json
...
export interface paths {
    "/api/auth/signup": {
        ...
        /**
         * Sign up
         * @description Create a new user account and start a session. The client supplies the user id. A verification email is sent to the address; the account is usable immediately and `email_verified` starts false. Every unexpired invitation outstanding for the address, across every project, takes effect here and the account joins those boards at the invited role.
         */
    ...
    "/api/invitations/accept": {
         ...
         * @description Redeem an invitation link and join the board it names. The caller must be signed in but need not be signed in as the invited address — an invitation is a grant to whoever holds the link, so someone who signs up under a different address can still accept. The invitation is consumed: a second attempt with the same token answers 422, as does one that was revoked, expired, or whose board has been deleted. An existing member is never demoted, so the role in the response is the effective one. There is no project id in the path because the holder of a link does not know it.
         */
