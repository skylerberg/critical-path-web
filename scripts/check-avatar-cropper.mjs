...
// flat, named colours, and asserts the one thing the unit tests cannot reach —
// that the square the canvas writes out is the square the circle was showing.
//
//   npm run check:avatar-cropper
//   node scripts/check-avatar-cropper.mjs --selftest
//
// Both halves of that comparison are outside jsdom: the display is CSS layout of
// an image sized in frame-widths, and the output is a canvas encode. Each can be
// read on its own and look right while the two disagree, which is exactly what
// they did — a pan slid the image off the frame and uncovered the background,
// while the file that came out was cropped correctly and told nobody.
//
...
//
// Each arm that moves the image gets its own page load where the one before it
// would otherwise still be in force, since nothing here puts the view back.
//
...
// Puts the component back on two bugs, both of which leave the saved file
// perfect and the display wrong — the asymmetry that makes the comparison arms
// worth their runtime, since every other arm here reads only the output.
const MUTATIONS = [
  {
  ...
  },
  {
    ...
    find: '`rotate(${String(rotation)}deg) scale(${String(zoom)})`',
    replace: '`scale(${String(zoom)})`',
...
const dragged = await saveAndSample();
check(
  'a drag moves the image under the circle',
  dragged.saved.length === POINTS.length && dragged.saved.every((color) => color === 'red'),
  JSON.stringify(dragged.saved)
);
check(
  'what a drag saves is what it was showing',
  agrees(dragged.saved, dragged.screen),
  `saved ${JSON.stringify(dragged.saved)} vs shown ${JSON.stringify(dragged.screen)}`
);
