...
export interface paths {
  ...
  '/api/task-series': {
     ...
     * @description A series holds the template for a repeating card — title, description, labels, assignees, checklist items, an optional due date and destination column — plus an RFC 5545 RRULE and the calendar day its next occurrence falls on. Nothing is created ahead of time and nothing appears early: a background sweep materializes an ordinary card on the day the occurrence falls, and then advances the schedule. The occurrence decides only when the card comes into existence — a card carries the template’s own `due_date`, never the occurrence date. Cards already created are ordinary cards and never change when the series does. Between occurrences there is no card, so this list is the only place a recurring commitment is visible. Viewers may read it; only editors may change it. Ordered by the next occurrence, soonest first, with paused and finished series last.
     */
    get: operations['getApiTaskSeries'];
    put?: never;
    /**
     * Create a recurring series
     * @description A series holds the template for a repeating card — title, description, labels, assignees, checklist items, an optional due date and destination column — plus an RFC 5545 RRULE and the calendar day its next occurrence falls on. Nothing is created ahead of time and nothing appears early: a background sweep materializes an ordinary card on the day the occurrence falls, and then advances the schedule. The occurrence decides only when the card comes into existence — a card carries the template’s own `due_date`, never the occurrence date. Cards already created are ordinary cards and never change when the series does. Send either `preset` (one of the curated recurrences) or a raw `rrule`, never both. A raw rule must be a single RRULE value carrying no DTSTART, TZID, RDATE, EXDATE or EXRULE — the anchor is `start_date` and the zone is `timezone` — and must repeat daily or less often. The first occurrence is scheduled on or after today in the series timezone, so a past `start_date` backfills nothing. A project holds at most 50 series. Image nodes cannot belong to a template and are stripped from the description; `dropped_image_count` reports how many. The client supplies the id; a duplicate returns 409.
     */
